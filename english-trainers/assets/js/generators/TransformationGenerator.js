...
class TransformationGenerator {
  ...
  _generateNegative() {
    ...
    return {
      question: `
        <div style="margin-bottom: 0.5rem;">Make this sentence <strong>negative</strong>:</div>
  ...
  _generateQuestion() {
    const pronouns = ['you', 'he', 'she', 'we', 'they']; // Exclude 'I' and 'it' for natural questions
    const pronoun = pronouns[Math.floor(Math.random() * pronouns.length)];
    const verb = this.verbMap[pronoun];
    const template = this.templates[Math.floor(Math.random() * this.templates.length)];
    ...
    return {
      question: `
        <div style="margin-bottom: 0.5rem;">Make this a <strong>question</strong>:</div>
        <div class="transformation-sentence" style="font-size: 1.1rem; padding: 1rem; background: rgba(0,0,0,0.2); border-radius: 8px; margin: 1rem 0;">
          "${sentence}"
        </div>
      `,
  ...
  _generatePlural() {
    ...
    return {
      question: `
        <div style="margin-bottom: 0.5rem;">Change to <strong>plural</strong>:</div>
        <div class="transformation-sentence" style="font-size: 1.1rem; padding: 1rem; background: rgba(0,0,0,0.2); border-radius: 8px; margin: 1rem 0;">
          "${sentence}"
        </div>
      `,
